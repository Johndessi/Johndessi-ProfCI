...
function construireInstructionsCultureLitteraireLycee({ titreOeuvre, auteurOeuvre, contenuFourni, situationApprentissage, numeroSeance, intituleOfficielSeance, biographieAuteur, themeOeuvre }) {
  ...
  // mouvement. Conclusion : corriger catégorie par catégorie au fil des
  // incidents ne suffit pas -- le modèle invente un fait précis plausible
  // dès qu'une catégorie n'est pas explicitement nommée dans l'interdiction,
  // quelle que soit cette catégorie. Cette version généralise donc
  // l'interdiction à toute affirmation factuelle précise et invérifiable
  // (pas seulement territoriale), avec des exemples couvrant les deux
  // incidents observés à ce jour.
  const interdictionFaitPrecisNonVerifie = ` INTERDICTION EXPLICITE : n'affirme JAMAIS un fait précis et invérifié sur un lieu, une époque, un mouvement/groupe ou une personne -- ceci inclut, sans s'y limiter : une classification administrative/politique/territoriale précise (rattachement à une fédération, statut de territoire/colonie, division administrative précise) ; la composition exacte d'un mouvement, courant ou groupe (qui l'a fondé, qui en fait partie) ; l'attribution d'une action, d'une œuvre ou d'un rôle à une personne nommée ; une date ou un événement précis. Pour tout point de ce type, reste général (nomme le pays/lieu actuel sans préciser son statut historique ; désigne un mouvement littéraire sans nommer ses fondateurs ou membres si tu n'es pas certain) -- SAUF si cette information figure mot pour mot dans des faits vérifiés fournis par ailleurs.`;
