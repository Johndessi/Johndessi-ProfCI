...
function construireInstructionsCultureLitteraireLycee({ titreOeuvre, auteurOeuvre, contenuFourni, situationApprentissage, numeroSeance, intituleOfficielSeance, biographieAuteur, themeOeuvre }) {
  ...
  // Interdiction nommée (22/09, incident Ferdinand Oyono/TleA) -- la simple
  // consigne "reste général" s'est révélée insuffisante en test réel : le
  // modèle a laissé passer une classification administrative/coloniale
  // fausse (Cameroun rattaché à l'AEF) alors même que cette consigne
  // s'appliquait déjà. Contrairement au bloc de faits vérifiés (qui ne
  // couvre que ce que la recherche web a effectivement trouvé), cette
  // interdiction s'applique TOUJOURS, y compris quand la recherche échoue.
  const interdictionClassificationPrecise = ` INTERDICTION EXPLICITE : n'affirme JAMAIS de classification administrative, politique ou territoriale précise sur un lieu ou une époque (rattachement à une fédération, statut de territoire/colonie, division administrative précise) -- nomme uniquement le pays/lieu actuel, sans préciser son statut ou son rattachement à une entité historique donnée, sauf si cette information figure mot pour mot dans des faits vérifiés fournis par ailleurs.`;
...
${construireConsigneAxeEtudeSituationOeuvreLycee(axe, situation)}`;
  ...
  const consigneBiographie = biographie
    ...
    : `1- Biographie : bref, JAMAIS un paragraphe développé -- 2 à 3 phrases maximum, uniquement l'essentiel : nationalité/identité, date de naissance (et de décès si l'auteur n'est plus vivant), profession, activité principale (distinctions/prix notables). À partir de tes connaissances réelles sur cet auteur -- si tu n'es pas certain d'un fait précis (date exacte, détail biographique), reste général plutôt que d'inventer un détail que tu ne connais pas avec certitude. INTERDICTION EXPLICITE (22/09, incident Ferdinand Oyono confondu avec l'Afrique équatoriale française) : n'affirme JAMAIS de classification administrative, politique ou territoriale précise sur le lieu de naissance (rattachement à une fédération ou un territoire colonial précis, statut administratif exact à une époque donnée) -- nomme uniquement le pays actuel (ex. "au Cameroun"), sans préciser son statut ou son rattachement à une entité de l'époque, sauf si cette information figure mot pour mot dans des faits vérifiés fournis par ailleurs.`;
