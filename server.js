...
//
// CONSTRUCTION DÉTERMINISTE, v2 (08/09) : v1 (16/08, "marqueurs de contenu +
// construction serveur des <tr>") a été testée en conditions réelles sur un
// échantillon adverse AVEC texte support fourni et semblait tenir (~80% de
// réussite mesuré alors) -- mais jamais testée dans le cas, pourtant le plus
// courant en usage réel (sélection par catalogue officiel, sans texte collé,
// cf. leconNecessiteTexteSupport qui n'est ici qu'un AVERTISSEMENT, jamais un
// blocage). Reproduction le 08/09 (5 essais identiques, 6e, catalogue,
// aucun texte support) : ~80% d'ÉCHEC, y compris sur des classes jusque-là
// jugées fiables -- le modèle doit alors À LA FOIS inventer un texte support
// ET produire, dans le MÊME appel géant (qui contient aussi toutes les
// instructions génériques de la fiche), un contenu vocabulaire/grammaire
// suivant un format qui n'est pas son biais naturel pour "analyser un texte
// français" (ce biais, lui, est Hypothèse générale/Axes -- exactement la
// structure de la Lecture méthodique, décrite juste à côté dans le code de
// cette appli). v1 ne protégeait que le TABLEAU (marqueurs de contenu) ; elle
// n'empêchait pas le résidu "Hypothèse générale :" observé HORS tableau
// (ex. dans la Situation d'apprentissage) sur un essai par ailleurs réussi.
//
// v2 isole ENTIÈREMENT la génération du contenu vocabulaire/grammaire dans
// un appel Anthropic SÉPARÉ et minuscule (cf. genererDeroulementExploitationAuto
// ci-dessous, même principe que rechercherInfosOeuvre pour la biographie
// d'auteur) : sortie JSON stricte, aucune mention de "hypothèse"/"axe"/
// Lecture méthodique nulle part dans ce prompt isolé, résultat injecté tel
// quel dans les <tr> (cf. construireLigneDeroulementHTML, même fonction que
// v1 et que le Mode 2 plan-enseignant). Le modèle principal (qui reçoit,
// lui, l'énorme prompt générique) n'a plus la charge de ce contenu du tout :
// il place UN SEUL marqueur, rien de plus -- son "biais Lecture méthodique"
// n'a donc plus aucune prise, y compris sur la Situation d'apprentissage
// (interdiction explicite étendue à tout le document, pas seulement au
// tableau). Cet appel isolé gère aussi la génération du texte support si
// aucun n'a été fourni (l'ancien mécanisme, cf. extraireTexteSupportGenereEnPlace,
// n'existe QUE pour Lecture méthodique/Résumé -- jamais porté à Exploitation
// de texte, seconde cause probable du taux d'échec constaté sans texte fourni).
...
async function genererDeroulementExploitationAuto({ texteSupport, lecon, classe, seance, avecRappelSeancePrecedente }) {
  ...
  const consigneTexte = texteFourni
...
TECHNIQUE D'EXPRESSION (optionnelle) : détermine D'ABORD si le texte contient réellement une figure de style parmi celles attendues à ce niveau (${figuresNiveau}) ou un procédé d'organisation textuelle notable (plan visible, connecteurs logiques structurants, énumération organisée). Si OUI : techniqueExpressionPresente=true, et "traces" contient le nom du procédé + un exemple précis relevé DANS le texte + l'explication de son effet -- C'EST ICI, ET ICI SEULEMENT, que ces mots doivent apparaître. Si NON (aucun point réel identifié) : techniqueExpressionPresente=false et techniqueExpression=null -- n'invente RIEN.
...
// la fiche -- le contenu vocabulaire/grammaire/évaluation est déjà rédigé
// (cf. genererDeroulementExploitationAuto ci-dessus) : le modèle principal
// n'a plus qu'à placer UN SEUL marqueur, jamais à rédiger ce contenu.
function construireInstructionsExploitationDeTexte({ sectionIIIIncluse }) {
  return `
...
// la ligne légitime (construite par le serveur, cf. genererDeroulementExploitationAuto)
...
// dupliquer.
const INTERDICTION_FAIT_PRECIS_NON_VERIFIE = ` INTERDICTION EXPLICITE : n'affirme JAMAIS un fait précis et invérifié sur un lieu, une époque, un mouvement/groupe, une personne, ou une édition -- ceci inclut, sans s'y limiter : une classification administrative/politique/territoriale précise (rattachement à une fédération, statut de territoire/colonie, division administrative précise) ; la composition exacte d'un mouvement, courant ou groupe (qui l'a fondé, qui en fait partie) ; l'attribution d'une action, d'une œuvre ou d'un rôle à une personne nommée ; une date ou un événement précis ; un détail éditorial/bibliographique précis (date ou lieu de l'édition originale, maison d'édition, numéro d'édition -- ne distingue pas une édition originale d'une réédition si tu n'es pas certain de laquelle est la première) ; un titre d'œuvre, de poème ou d'extrait textuel précis présenté comme un exemple concret à lire, citer ou analyser (n'invente JAMAIS un titre de poème, de texte ou d'extrait à faire étudier ou analyser aux élèves s'il ne t'a pas été fourni -- pour illustrer une forme, une notion ou une technique, décris-la ou désigne-la de façon générique, sans lui accoler de titre : « un sonnet », « un exemple de ce type de poème », jamais « le sonnet intitulé X » ou un titre inventé) ; la qualification d'un auteur nommé par un mouvement, courant ou école littéraire précis (ex. « poète romantique », « écrivain symboliste », « auteur réaliste ») lorsque cette qualification n'a pas été fournie telle quelle par l'enseignant -- un même auteur pouvant être rattaché différemment selon les critiques ou les époques de sa carrière, ne tranche JAMAIS ce point toi-même, même si tu penses le savoir avec certitude. Pour tout point de ce type, reste général (nomme le pays/lieu actuel sans préciser son statut historique ; désigne un mouvement littéraire sans nommer ses fondateurs ou membres si tu n'es pas certain ; mentionne l'œuvre sans préciser une date d'édition si tu n'es pas certain qu'il s'agit bien de l'édition originale ; désigne un exemple textuel de façon générique sans lui inventer de titre ; désigne un auteur par son nom seul, sans lui accoler une étiquette de mouvement/école non fournie) -- SAUF si cette information figure mot pour mot dans des faits vérifiés fournis par ailleurs.`;
...
- SVT / PHYSIQUE-CHIMIE : inclus expériences, schémas descriptifs, observations, conclusions -- toute donnée chiffrée (mesure, résultat d'expérience, statistique) ou tout nom de scientifique cité doit être réel et exact, ou explicitement présenté comme un exemple hypothétique ("supposons que...", "dans cette expérience fictive...")
- HISTOIRE-GÉO : inclus documents sources, cartes, questions d'exploitation -- tout personnage, toute date et tout événement historique cité doit être réel et exact (jamais un nom, une date ou un événement plausible mais inventé)
...
// Formulation exacte extraite de la fiche de référence "Maeva" (Fatou
// Fanny-Cissé, 3e, DPFC) -- corrigée le 01/09 : la formulation précédente
// ("Traiter des situations relatives à...") était une paraphrase, pas le
// libellé officiel réel.
...
// Libellé du champ Séance (entête), calibré sur la fiche de référence
// "Maeva" : les séances 2-9 ne portent PAS un intitulé générique de
// contenu -- juste le sous-type et son rang PARMI LES SÉANCES DE MÊME
// SOUS-TYPE de la séquence ("Lecture suivie n°1", "Lecture méthodique n°1",
// "Lecture suivie n°2"...), calculé à partir de la planification complète
// (planSeancesOI, déjà validée à ce stade). Séances 1/10/11 : libellés
// fixes, alignés mot pour mot sur la fiche de référence pour 1 et 10
// (11 n'y figure pas -- structure confirmée séparément par l'enseignant,
// cf. point 6 de la note qualité du 01/09).
function construireSeanceAfficheeOeuvre(seanceNum, planSeancesOI) {
  if (seanceNum === 1) return "1 : Introduction à l'étude de l'œuvre intégrale";
  if (seanceNum === 10) return '10 : Conclusion à l\'étude de l\'œuvre intégrale';
  if (seanceNum === 11) return '11 : Évaluation finale';
  if (seanceNum >= 2 && seanceNum <= 9) {
    const plan = Array.isArray(planSeancesOI) ? planSeancesOI : [];
    const type = plan[seanceNum - 2];
    const libelleType = type === 'lecture_methodique' ? 'Lecture méthodique' : 'Lecture suivie';
    const ordinal = plan.slice(0, seanceNum - 1).filter((t) => t === type).length;
    return `${seanceNum} : ${libelleType} n°${ordinal}`;
  }
  return String(seanceNum);
}
...
${consigneTexteSupport}DÉVELOPPEMENT -- OBLIGATION STRUCTURELLE ABSOLUE : le tableau DÉROULEMENT doit contenir EXACTEMENT 4 lignes <tr> SÉPARÉES pour le développement -- une ligne <tr> par étape I, II, III, IV, JAMAIS fusionnées en une seule ligne ni regroupées à plusieurs, même si le contenu d'une étape tient en une phrase courte. Exactement comme la ligne PRÉSENTATION et la ligne ÉVALUATION ont chacune leur propre <tr>, chacune des 4 étapes I à IV a SA PROPRE ligne <tr> : au total le tableau DÉROULEMENT compte donc 6 lignes de contenu (PRÉSENTATION, I, II, III, IV, ÉVALUATION), jamais moins. Chaque ligne <tr> du développement porte les 5 colonnes standard (Moments didactiques/Durée | Stratégies pédagogiques/Plan du cours | Activités de l'enseignant | Activités des élèves | Traces écrites) et sa colonne "Moments didactiques/Durée" commence par le numéro romain de l'étape (I., II., III., IV.). Reproduis FIDÈLEMENT le plan ci-dessous rédigé par l'enseignant -- sans changer le sens, sans y ajouter ni en retirer aucun élément, reformaté seulement pour l'intégrer à ces colonnes :

I. PRÉSENTATION DU TEXTE : en Activités de l'enseignant/des élèves, questions-réponses amenant le titre, l'œuvre d'origine, la date de publication et l'auteur tels que fournis par l'enseignant dans son plan. En Traces écrites, la présentation doit être rédigée en PROSE, EXACTEMENT sur le modèle suivant (jamais sous forme de lignes séparées "Titre : ... / Auteur : ...") : « Le texte qui fera l'objet de notre étude est intitulé « [Titre] ». Il est extrait de [Œuvre], publié(e) en [date]. Il est écrit/composé par [Auteur]. » -- remplace chaque crochet par l'information correspondante fournie par l'enseignant, choisis "écrit" ou "composé" selon ce qui convient au genre du texte, et n'ajoute AUCUNE information absente du plan (ni genre, ni nature du texte, ni autre précision non fournie). Ne reproduis JAMAIS le texte intégral de l'œuvre ici, seulement sa présentation.
...
ÉVALUATION (ligne distincte du tableau DÉROULEMENT, séparée du Bilan général -- ne jamais fusionner les deux) : reprends l'évaluation telle que fournie par l'enseignant dans son plan ; à défaut, rédige TOI-MÊME quelques questions individuelles à l'écrit testant UNIQUEMENT le contenu réel des axes déjà reproduits en III (les analyses et interprétations déjà données, jamais une citation ou un passage du texte non déjà cité intégralement) -- jamais un nouveau texte, jamais une notion absente du plan de l'enseignant.
   RÈGLE ABSOLUE ANTI-INVENTION (ÉVALUATION) : l'évaluation est un ENSEMBLE DE QUESTIONS SEULEMENT -- ne fournis JAMAIS de corrigé, de "correction collective" ou de réponses-modèles, même à titre d'exemple. Ne complète JAMAIS une citation tronquée par "..." dans le plan de l'enseignant (ex. "le sang... ouvertes") par le texte que tu penses manquant, même si tu crois connaître l'œuvre -- une citation tronquée dans le plan reste tronquée partout où elle est réutilisée, y compris dans les questions d'évaluation. Si une question a besoin de renvoyer à un passage précis, renvoie-y par sa référence (numéro de vers, entrée, axe) et non par une citation complétée de ton cru.${consignePerimetre}
...
function construireInstructionsIntroductionOeuvre({ titreOeuvre, auteurOeuvre, etablissement, axeEtude, analyseCouverture, themeOeuvre, personnagesOeuvre, lieuxOeuvre, biographieAuteur }) {
  ...
  // intégralement le squelette générique (tableau Habiletés/Contenus +
  // 3 phases Présentation/Développement/Évaluation) pour cette séance
  // UNIQUEMENT, exactement comme le fait déjà la Lecture méthodique
  // classique pour son propre squelette dédié.
  const consigneCouverture = couverture
...
function construireInstructionsConclusionOeuvre({ titreOeuvre, auteurOeuvre, axeEtude }) {
  ...
  // Structure I-II-III calibrée MOT POUR MOT sur la fiche de référence
  // "Maeva" (Thèmes abordés / Jugement critique / Schéma actantiel),
  // fournie le 01/09 -- remplace intégralement le squelette générique pour
...
// Situation d'évaluation + Traitement de l'unité significative : calibré
// mot pour mot sur la fiche de référence "Maeva" (01/09) -- rattachée à la
// DERNIÈRE unité significative de la séance (jamais une section Évaluation
// séparée), questions et réponses numérotées, saisies par l'enseignant.
function construireSituationEvaluationHTML(questionsEvaluation, reponsesEvaluation) {
  const questions = (Array.isArray(questionsEvaluation) ? questionsEvaluation : []).filter(Boolean);
  const reponses = (Array.isArray(reponsesEvaluation) ? reponsesEvaluation : []).filter(Boolean);
  if (!questions.length && !reponses.length) return '';
  const questionsHTML = questions.map((q, i) => `${i + 1}-${echapperHtml(q.toString().trim())}`).join(', ');
  const reponsesHTML = reponses.map((r, i) => `${i + 1}-${echapperHtml(r.toString().trim())}`).join('<br>');
  return `<p><strong>Situation d'évaluation</strong><br>${questionsHTML}</p>
<p><strong>Traitement de l'unité significative.</strong><br>${reponsesHTML}</p>`;
}

// Assemble déterministiquement les unités significatives saisies par
// l'enseignant (pages, citation des bornes du passage, titre, résumé en
// prose) -- jamais rédigées par le modèle, calibré mot pour mot sur la
// fiche de référence "Maeva" (01/09) : "Unité significative N : pp X-Y
// « premiers mots…derniers mots »" + "Titre : ..." + résumé narratif en
// prose (PAS une liste à puces de faits, contrairement à la version
// initiale du 01/09, corrigée après lecture de la fiche de référence). La
// Situation d'évaluation est rattachée à la dernière unité uniquement.
function construireUnitesSensHTML(unitesSens, questionsEvaluation, reponsesEvaluation) {
  ...
  return unites.map((u, i) => {
    ...
    const citation = echapperHtml((u && u.citation ? u.citation : '').toString().trim());
    const titre = echapperHtml((u && u.titre ? u.titre : `Unité ${i + 1}`).toString().trim());
    const resume = echapperHtml((u && u.resume ? u.resume : '').toString().trim());
    const estDerniere = i === unites.length - 1;
    return `<p><strong>Unité significative ${i + 1} : pp ${pages}${citation ? ` « ${citation} »` : ''}</strong><br>
<strong>Titre :</strong> ${titre}<br>
${resume}</p>
${estDerniere ? construireSituationEvaluationHTML(questionsEvaluation, reponsesEvaluation) : ''}`;
  }).join('\n');
}

// I- SITUATION DU PASSAGE / II- CONSTRUCTION DES UNITÉS SIGNIFICATIVES /
// III-BILAN -- entièrement déterministe (titres de sections + phrase de
// situation construite à partir de titreOeuvre/auteurOeuvre/passagePages/
// contexteNarratif, tous déjà connus côté serveur ou saisis par
// l'enseignant), calibré mot pour mot sur la fiche de référence "Maeva"
// (01/09), injecté à la place du jeton {{DEVELOPPEMENT_LECTURE_SUIVIE}}
// (cf. injecterMarqueurUneFois). Contrairement à la version initiale du
// 01/09, l'axe d'étude n'est PAS rappelé ici (absent de la fiche de
// référence pour cette section -- il ne l'est qu'en Séance 1 et 10).
function construireDeveloppementLectureSuivieHTML({ titreOeuvre, auteurOeuvre, passagePages, contexteNarratif, unitesSens, bilanSynthese, questionsEvaluation, reponsesEvaluation }) {
  const titre = echapperHtml((titreOeuvre || '').toString().trim());
  const auteur = echapperHtml((auteurOeuvre || '').toString().trim());
  const pages = echapperHtml((passagePages || '').toString().trim());
  const contexte = echapperHtml((contexteNarratif || '').toString().trim());
  return `<p><strong>I- SITUATION DU PASSAGE</strong><br>
Le passage à étudier se situe de la page ${pages} de « ${titre} » de ${auteur}. Il présente ${contexte}</p>
<p><strong>II- CONSTRUCTION DES UNITÉS SIGNIFICATIVES</strong></p>
${construireUnitesSensHTML(unitesSens, questionsEvaluation, reponsesEvaluation)}
<p><strong>III-BILAN</strong><br>
${echapperHtml((bilanSynthese || '').toString().trim())}</p>`;
}
...
function construireFicheLibreOeuvreSeance11({ discipline, classe, duree, titreOeuvre, auteurOeuvre, numeroSequence, contenuLibre }) {
  const leconAffichee = construireLeconAfficheeOeuvre(numeroSequence, titreOeuvre, auteurOeuvre);
  const seanceAffichee = construireSeanceAfficheeOeuvre(11, null);
  const corpsLibre = texteSupportVersHtml(contenuLibre);
  return `<div class="fiche-cours">
...
<div class="entete-libre" style="display:grid;grid-template-columns:110px 1fr;column-gap:12px;row-gap:2px;margin-bottom:14px;">
  ...
  <div style="font-weight:bold;padding:2px 0;">Séance :</div><div style="padding:2px 0;">${echapperHtml(seanceAffichee)}</div>
</div>
...
function limiterGenerationParIp(req, res, next) {
 ...
 app.post('/api/generer-fiche', limiterGenerationParIp, uploadTexteSupportFichier, async (req, res) => {
  ...
  try {
    ...
    const questionsEvaluationOI = parseTableauJSON(req.body.questionsEvaluationOI);
    const reponsesEvaluationOI = parseTableauJSON(req.body.reponsesEvaluationOI);
    ...
    // Mode 1 (automatique) d'Exploitation de texte, v2 (08/09) : le contenu
    // du tableau DÉROULEMENT est généré par un appel isolé AVANT le streaming
    // principal (cf. genererDeroulementExploitationAuto), jamais extrait de
    // marqueurs dans la réponse du modèle principal -- exploitationAutoResultat
    // porte ce contenu déjà construit jusqu'au post-traitement du stream.
    // modeAutoExploitationDeterministe reste true UNIQUEMENT quand ce mode
    // est effectivement déclenché (jamais en Mode 2, où la structure vient
    ...
    if (estOeuvreIntegrale && !profilInfoOI) {
      ...
      seanceAfficheeOI = construireSeanceAfficheeOeuvre(seanceNumOI, planSeancesOI);
      ...
      if (seanceNumOI === 1) {
        ...
        if (typeSeanceOI === 'lecture_methodique') {
          ...
          developpementLectureSuivieHTML = construireDeveloppementLectureSuivieHTML({
            titreOeuvre, auteurOeuvre, passagePages, contexteNarratif, unitesSens, bilanSynthese,
      ...
      if (estLM) {
        ...
        // lieu de le régénérer. Mode 1, v2 (08/09) : le contenu I/II/III/EVAL
        // est généré par un appel isolé AVANT le prompt principal, cf.
        // genererDeroulementExploitationAuto -- pas de blocage par
        ...
        // (figureStyleParNiveauCollege), désormais utilisée à l'intérieur de
        // genererDeroulementExploitationAuto.
        if (planCoursEstSubstantiel(planCours)) {
